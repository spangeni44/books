...
class HomeController extends Controller
{
    ...
    {
        $banner2=Banner::where('position',2)->where('published',1)->first();
        $sliders=Slider::all();
        $hardBooks=Product::where('category_id',38)->get();
        $featuredProducts=Product::where('featured',1)->orderBy('created_at','desc')->get();
        return view('frontend.index',compact('sliders','banner2','hardBooks','featuredProducts'));
    }
    ...
    public function product(Request $request, $slug)
    {
        ...
        // dd($detailedProduct->stocks);
         $discountValue = 0;
            ...
            $discount_type= $detailedProduct['discount_type'];
        ...
        if($detailedProduct!=null && $detailedProduct->published){
            ...
            //     // dd('test');
            //     return view('frontend.digital_product_details', compact('detailedProduct','discountValue'));
            // }
            // else {
            //     return view('frontend.product_details', compact('detailedProduct','discountValue'));
            // }
    ...
    public function productDetailAjax(Request $request){
        if(isset($request->slug)){
                ...
                $detailedProduct=Product::where('slug',$request->slug)->first();
				$html=view('frontend.inc.quickview')->with(compact('detailedProduct'));
    ...
    {
        $categories = Category::all();
        return view('frontend.all_publication', compact('categories'));
    }
    ...
    public function search_home(Request $request) {
        ...
        $products = Product::where('name',  'like', '%'.$search.'%')
                                 ...
                                 ->orWhere('tags',  'like', '%'.$search.'%')->paginate(5);
        }
    ...
    public function search(Request $request)
    {
        ...
        $publication_id = (Publication::where('slug', $request->publication)->first() != null) ? Publication::where('slug', $request->publication)->first()->id : null;
        ...
        if($publication_id != null){
            $conditions = array_merge($conditions, ['publication_id' => $publication_id]);
        }
        if($category_id != null){
        ...
        if(isset($request->per_page_item)){
            $products = filter_products($products)->paginate($request->per_page_item)->appends(request()->query());
        }else{
            $products = filter_products($products)->paginate(16)->appends(request()->query());
        }
        $per_page_item=$request->per_page_item;
        return view('frontend.search', compact('products', 'query', 'category_id', 'subcategory_id', 'subsubcategory_id', 'publication_id', 'sort_by', 'per_page_item','seller_id','min_price', 'max_price', 'attributes', 'selected_attributes', 'all_colors', 'selected_color'));
    }
    ...
    public function variant_price(Request $request)
    {
        ...
        if($product->shipping_type=='flat_rate'){
            ...
            if($shipping_address=='outside'){
                $delivery_charge = $shipping_cost +50;
            }else{
                $delivery_charge = $shipping_cost;
            }
        }
        return array('price' => single_price($price*$request->quantity), 'quantity' => $quantity, 'digital' => $product->digital,'delivery_charge'=>$delivery_charge);
    }
    ...
    //         $shipping_cost= $product->shipping_cost;
    //         if($shipping_address=='outside'){
    //             $updated_price = $shipping_cost +50;
    //         }else{
    //             $updated_price = $shipping_cost;
    //         }
            
    //         //extract int from string
    //         $price_int=preg_replace('/[^\d.]/', '', $price);
    //         // preg_match_all('!\d+!', $price, $price_int);
    //         $total_price = $updated_price+$price_int;
    //         return response()->json(['price' =>'Rs '. $total_price]);
    //     }
    ...
    public function get_category_items(Request $request){
     ...
     public function get_all_category_items(){
        $category=Category::all();
        return view('frontend.partials.all_category_elements',compact('category'));
    }
    ...
    {
        $products = Product::where('user_id', Auth::user()->id)->where(['digital'=>1,'book_type'=>"ebook"])->orderBy('created_at', 'desc')->paginate(10);
    ...
    public function seller_audio_product_list(Request $request)
    {
        $products = Product::where('user_id', Auth::user()->id)->where(['digital'=>1,'book_type'=>'audio'])->orderBy('created_at', 'desc')->paginate(10);
        return view('frontend.seller.audioproducts.products', compact('products'));
    }
    public function show_audio_product_upload_form(Request $request)
    {
        if(\App\Addon::where('unique_identifier', 'seller_subscription')->first() != null && \App\Addon::where('unique_identifier', 'seller_subscription')->first()->activated){
            if(Auth::user()->seller->remaining_digital_uploads > 0){
                $business_settings = BusinessSetting::where('type', 'digital_product_upload')->first();
                $categories = Category::where('digital', 1)->get();
                return view('frontend.seller.audioproducts.product_upload', compact('categories'));
            }
            else {
                flash(translate('Upload limit has been reached. Please upgrade your package.'))->warning();
                return back();
    ...
    public function show_audio_product_edit_form(Request $request, $id)
    {
        $categories = Category::where('digital', 1)->get();
        $product = Product::find(decrypt($id));
        return view('frontend.seller.audioproducts.product_edit', compact('categories', 'product'));
    }
    ...
    public function allBlogs(Request $request){
     ...
     public function getBlogDetails(Request $request){
         ...
         $search = null;
        if(isset($request->slug)){
            $blog=Blog::where(['slug'=>@$request->slug,'status'=>'active'])->first();
