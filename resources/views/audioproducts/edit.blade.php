...
@section('content')
	...
	<div class="row">
		<div class="col-lg-8 col-lg-offset-2">
			<form class="form form-horizontal mar-top" action="{{route('audioproducts.update', $product->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
				...
				<div class="panel">
					...
					<div class="panel-body">
						<div class="form-group">
							<label class="col-lg-2 control-label">{{translate('Book Name')}}</label>
							<div class="col-lg-7">
								<input type="text" class="form-control" name="name" placeholder="{{translate('Book Name')}}" value="{{$product->name}}" required>
							</div>
						</div>
						<div class="form-group">
    						<label class="col-lg-2 control-label">{{translate('Book Version')}}</label>
    						<div class="col-lg-7">
    							<input type="number" class="form-control" name="book_version" value="@$product->book_version" required>
    						</div>
    					</div>
						<div class="form-group">
    						<label class="col-lg-2 control-label">{{translate('Book Author(s)')}}</label>
    						<div class="col-lg-7">
    							<input type="text" class="form-control" name="author_name[]" value="{{@$product->author_name}}" placeholder="{{ translate('Type to add author') }}" data-role="tagsinput">
    						</div>
    					</div>
    						<div class="form-group">
    						<label class="col-lg-2 control-label">{{translate('Book Reader(s)')}}</label>
    						<div class="col-lg-7">
    							<input type="text" class="form-control" name="book_reader[]" value="{{@$product->book_reader}}" placeholder="{{ translate('Type to add book readers') }}" data-role="tagsinput">
    						</div>
    					</div>
						...
						<div class="form-group" id="publication">
                            <label class="col-lg-2 control-label">{{translate('Publication Name')}}</label>
                            <div class="col-lg-7">
                                <select class="form-control demo-select2-placeholder" name="publication_id" id="publication_id">
    								...
    								@foreach (\App\Publication::all() as $publication)
    									<option value="{{ $publication->id }}" @if($product->publication_id == $publication->id) selected @endif>{{ $publication->name }}</option>
    								@endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
    						<label class="col-lg-2 control-label">{{translate('Publish Date')}}</label>
    						<div class="col-lg-7">
    							<input type="date" class="form-control" value="{{@$product->publish_date}}" name="publish_date" required>
    						</div>
    					</div>
    					<div class="form-group">
    						<label class="col-lg-2 control-label">{{translate('Trial Time(in minutes) ')}}</label>
    						<div class="col-lg-7">
    							<input type="number" class="form-control" name="trial_time" value="{{@$product->trial_time}}" required>
    						</div>
    					</div>
						<div class="form-group">
    						<label class="col-lg-2 control-label">{{translate('No of Pages')}}</label>
    						<div class="col-lg-7">
    							<input type="number" class="form-control" name="no_of_pages" value="{{@$product->no_of_pages}}" min="1" required>
    						</div>
					    </div>
					    <div class="form-group">
    						<label class="col-lg-2 control-label">{{translate('Duration')}}</label>
    						<div class="col-lg-7">
    							<input type="number" class="form-control" name="duration" value="{{@$product->duration}}" min="1" required>
    						</div>
					    </div>
						<div class="form-group">
							<label class="col-lg-2 control-label">{{translate('Audio File')}}</label>
							<div class="col-lg-7">
				...
				<div class="panel">
					...
					<div class="panel-body">
						...
						<div class="form-group">
							<label class="col-lg-2 control-label">{{translate('Price Per Day')}}</label>
							<div class="col-lg-7">
								<input type="number" min="0" value="{{@$product->price_per_day}}" step="0.01" placeholder="{{translate('Per Day Price')}}" name="price_per_day" class="form-control" required>
							</div>
						</div>
						<div class="form-group">
							<label class="col-lg-2 control-label">{{translate('Price Per Week')}}</label>
							<div class="col-lg-7">
								<input type="number" min="0" value="{{@$product->price_per_week}}" step="0.01" placeholder="{{translate('Per Day Week')}}" name="price_per_week" class="form-control" required>
							</div>
						</div>
						<div class="form-group">
							<label class="col-lg-2 control-label">{{translate('Price Per month')}}</label>
							<div class="col-lg-7">
								<input type="number" min="0" value="{{@$product->price_per_month}}" step="0.01" placeholder="{{translate('Per Day month')}}" name="price_per_month" class="form-control" required>
							</div>
						</div>
						<div class="form-group">
