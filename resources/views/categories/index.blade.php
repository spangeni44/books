            ...
            <tbody>
                @foreach($categories as $key => $category)
                    <tr>
                        ...
                        <td><label class="switch">
                            <input onchange="update_top_cat(this)" value="{{ $category->id }}" type="checkbox" <?php if($category->top_cat == 1) echo "checked";?> >
                            <span class="slider round"></span></label></td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_top_cat(el){
            ...
            $.post('{{ route('categories.top_cat') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Top Menu updated successfully');
                }
                else{
                    showAlert('danger', 'Something went wrong');
                }
