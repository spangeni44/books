...
<div class="panel">
    <div class="panel-heading bord-btm clearfix pad-all h-100">
        <h3 class="panel-title pull-left pad-no">{{translate('Offline Customer Package Payment Requests')}}</h3>
    </div>
            ...
            <tbody>
                @foreach($package_payment_requests as $key => $package_payment_request)
                    <tr>
                        ...
                        <td>{{ $package_payment_request->customer_package->name }}</td>
        ...
        function offline_payment_approval(el){
            ...
            $.post('{{ route('offline_customer_package_payment.approved') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    ...
                    showAlert('success', 'Offline Customer Package Payment approved successfully');
                }
                else{
