    ...
    <div class="panel">
    	<div class="panel-body">
    			...
    			<div class="col-sm-6 text-xs-center">
    				<address>
        				<strong class="text-main">{{ json_decode($order->shipping_address)->name }}</strong><br>
                         ...
                         <b>@if(isset($order->shipping_charge_type)) {{ ucwords(str_replace('_',' ',@$order->shipping_charge_type)) }} @if(isset(json_decode($order->shipping_address)->inside_valley_address)) ,{{ucwords(str_replace('_',' ',json_decode($order->shipping_address)->inside_valley_address))}} @endif @endif</b><br>
        				 {{ json_decode($order->shipping_address)->address }}, {{ json_decode($order->shipping_address)->city }}, @if(isset(json_decode($order->shipping_address)->postal_code)) {{ json_decode(@$order->shipping_address)->postal_code }} @endif<br>
                         {{ ucwords(json_decode($order->shipping_address)->country) }}
                    </address>
    		...
    		<div class="clearfix">
    			...
    			<tr>
    				<td>
    					<strong>{{translate('TOTAL (including tax)')}} :</strong>
    				</td>
